describe('Test cc-tantocd-input', function() {
    ...
    beforeEach(inject(function($injector, $sniffer) {
        ...
        $httpBackend.when('GET', '/core/codename/m016tanm/123456789?hakkoDay=19010101').respond([record1]);
        $httpBackend.when('GET', '/core/codename/m016tanm/123456789?hakkoDay=20140101').respond([record3]);
        $httpBackend.when('GET', '/core/codename/m016tanm/123456780?hakkoDay=19010101').respond([record2]);
        $httpBackend.when('GET', '/core/codename/m016tanm/111111111?hakkoDay=19010101').respond([]);
        ...
        compile = function(label, partition, id, name, ngModel, required, readOnly, notExist, delexist,
                para01, id2, name2, ngModel2) {

            var htmlText = '<cc-tantocd-input' + ' cc-label="' + label + '" cc-partition="' + partition + '" id="'
    ...
    describe('display html correctly cases', function() {
        ...
        describe('1st div cases', function() {
            it("should render html of 1st div correctly with required set to true", function() {
                compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", "true", null, "true", "false",
                        "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
                ...
                expect(element.find('div').eq(0).clone().children().remove().end().text()).toEqual("担当者");
            ...
            it("should render html of 1st div correctly with required set to false", function() {
                compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", 'false', null, "true", "false",
                        "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
                expect(element.find('div').length).toEqual(1);
            ...
            it("should render html of 1st div correctly with readonly set to true", function() {
                compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", null, "true", "true", "false",
                        "result.hakkoDay", "tantoNm", "result.tantoNm");
                expect(element.find('div').length).toEqual(1);
                expect(element.find('div').eq(0).hasClass("cs-label-large cs-label")).toEqual(true);
                expect(element.find('div').eq(0).clone().children().remove().end().text()).toEqual("担当者");
                expect(element.find('div span').length).not.toEqual(1);
            ...
            it("should render html of 1st div correctly with partition set to empty", function() {
                compile("担当者", "", "tantoCd", "tantoCd", "result.tantoCd", "true", null, "true", "false",
                        "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
                expect(element.find('div').length).toEqual(1);
        ...
        describe('1st input field', function() {
            ...
            it("should render html correctly when required", function() {
                ...
                expect(element.find('input').eq(0).attr('id')).toEqual("tantoCd");
                expect(element.find('input').eq(0).attr('name')).toEqual("tantoCd");
                ...
                expect(element.find('input').eq(0).attr('maxlength')).toEqual("9");
            });
            
            it("should render html correctly when readonly", function() {
                compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", null, "true", null, "false",
                        "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
        ...
        describe('2nd input field', function() {
            ...
            it("should render html correctly", function() {
                ...
                expect(element.find('input').eq(1).attr('id')).toEqual("tantoNm");
                expect(element.find('input').eq(1).attr('name')).toEqual("tantoNm");
     ...
     describe("logic cases", function() {
        it('it should bind and format view value correctly', function() {
            compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", "true", null, "true", "true",
                    "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
            inputElement = element.find('input').eq(0);
            var modelCtrl = inputElement.controller('ngModel');
            
            inputElement.triggerHandler('focus');
            changeInputValue(inputElement, '123456789');
            $httpBackend.flush();
            
            inputElement.triggerHandler('blur');
            expect(inputElement.val()).toEqual('123456-789');
            expect(scope.result.tantoCd).toEqual('123456789');
            expect(modelCtrl.$valid).toBe(true);

            inputElement.triggerHandler('focus');
            expect(inputElement.val()).toEqual('123456789');
            expect(scope.result.tantoCd).toEqual('123456789');
            
        });
        
        it("it should catch required validation", function() {
            compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", "true", null, "true", "true",
                    "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
            inputElement = element.find('input').eq(0);
            var modelCtrl = inputElement.controller('ngModel');
            ...
            inputElement.triggerHandler('blur');
            expect(scope.result.tantoCd).toEqual(undefined);
            expect(modelCtrl.$invalid).toBe(true);
            expect(modelCtrl.$error.required).toBe(true);
            
        });
        
        it('it should catch format validation ', function() {
            compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", "true", null, "true", "true",
                    "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
             inputElement = element.find('input').eq(0);
             var modelCtrl = inputElement.controller('ngModel');
             ...
             expect(scope.result.tantoCd).toEqual(undefined);
             expect(modelCtrl.$invalid).toBe(true);
             expect(modelCtrl.$error.ccTantocd).toBe(true);
        ...
        it('it should populate input no2 with data, hakkoday exists ', function() {
            ...
            inputElement.triggerHandler('blur');
            expect(inputElement2.val()).toEqual('name3');
            expect(scope.result.tantoNm).toEqual("name3");
        ...
        it('it should populate input no2 with data, hakkoday doesnot exist ', function() {
            ...
            inputElement.triggerHandler('blur');
            expect(inputElement2.val()).toEqual('name1');
            expect(scope.result.tantoNm).toEqual("name1");
        ...
        it('it should not populate input no2 with data and display error message when there is no such data in database and notExist set to false', function() {
            compile("担当者", "3", "tantoCd", "tantoCd", "result.tantoCd", "true", null, "false", "false",
                    "result.hakkoDay", "tantoNm", "tantoNm", "result.tantoNm");
            inputElement = element.find('input').eq(0);
            inputElement2 = element.find('input').eq(1);
            var modelCtrl = inputElement.controller('ngModel');
            ...
            inputElement.triggerHandler('focus');
            changeInputValue(inputElement, '111111111');
            $httpBackend.flush()

            inputElement.triggerHandler('blur');
            expect(inputElement2.val()).toEqual('');
            expect(scope.result.tantoNm).toEqual(undefined);
            expect(modelCtrl.$invalid).toBe(true);
        ...
        it('it should not populate input no2 with data and display message when there is such data with kubun 9 and notDelexist set to false', function() {
            ...
            inputElement.triggerHandler('focus');
            changeInputValue(inputElement, '123456780');
            $httpBackend.flush()
