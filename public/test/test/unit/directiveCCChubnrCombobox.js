...
describe('Test directive cc-chubnr-combobox', function() {
    ...
    beforeEach(inject(function($injector, $sniffer) {
        ...
        list1 = [{code:'0001',name:'デリカ原料（仕入）1',shortName:'デリカ原料1',kubun:'1'},
                 {code:'0009',name:'デリカ原料（仕入）9',shortName:'デリカ原料9',kubun:'9'}];
        list2 = [{code:'1001',name:'チルドドリンク（催事）1',shortName:'チルドリ催1',kubun:'1'},
                 {code:'1002',name:'チルドドリンク（催事）2',shortName:'チルドリ催2',kubun:'2'},
                 {code:'1003',name:'チルドドリンク（催事）3',shortName:'チルドリ催3',kubun:'3'}];
        empty = [];

        $httpBackend.when('GET', '/core/codename/m005bnrm4/01/001?hakkoDay=19010101').respond(list1);
        $httpBackend.when('GET', '/core/codename/m005bnrm4/01/001?hakkoDay=20140101').respond(list2);
    ...
    describe('cc-chubnr-combobox', function() {
        // test template
        it('it should render right html', function() {

            // Test partition != '', required = true, shortName = false, delexist = false
            compile('ライン','3','chubnrCd','chubnrCd','cond.chubnrCd','true',null,'false','false', 
                    'cond.igyobuCd', 'cond.bmnCd', 'cond.hakkoDay');
            ...
            expect(selectElement.attr('id')).toEqual("chubnrCd");
            expect(selectElement.attr('name')).toEqual("chubnrCd");
            ...
            // Test partition == ''
            compile('ライン','','chubnrCd','chubnrCd','cond.chubnrCd','true',null,'false','false', 
                    'cond.igyobuCd', 'cond.bmnCd', 'cond.hakkoDay');
            ...
            // Test required == 'false'
            compile('ライン','3','chubnrCd','chubnrCd','cond.chubnrCd','false',null,'false','false', 
                    'cond.igyobuCd', 'cond.bmnCd', 'cond.hakkoDay');
            ...
            // Test readonly = true
            compile('ライン','','chubnrCd','chubnrCd','cond.chubnrCd',null,'true','false','false', 
                    'cond.igyobuCd', 'cond.bmnCd', 'cond.hakkoDay');
            //-----------it should hasn't span element
            expect(element.has('div').length).toEqual(1);
            expect(element.has('span').length).toEqual(0);
            ...
            // Test shortName = true
            compile('ライン','3','chubnrCd','chubnrCd','cond.chubnrCd','true',null,'false','true', 
                    'cond.igyobuCd', 'cond.bmnCd', 'cond.hakkoDay');
        ...
        it('it should be right binding value and view value', function() {
            ...
            expect(scope.cond.chubnrCd).toEqual('');
...
//            expect(selectElement.text()).toEqual('');
            ...
            expect(scope.cond.chubnrCd).toEqual(list1[0].code);
            ...
            // delexist = false
            compile('ライン','3','chubnrCd','chubnrCd','cond.chubnrCd','true',null,'true','false', 
                    'cond.igyobuCd', 'cond.bmnCd', 'cond.hakkoDay');
            $httpBackend.flush();
            selectElement = angular.element(element.children()[1]);
            var children = selectElement.children();
        ...
        it('it should be catch ccSelect and required validation ', function() {
            ...
            // value of scope.cond.chubnrCd out of listRecord
            scope.cond.chubnrCd = '0000';
            //scope.$digest();
