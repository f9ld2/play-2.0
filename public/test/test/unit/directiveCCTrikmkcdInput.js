describe('Test cc-trikmkcd-input', function() {
    ...
    beforeEach(inject(function($injector, $sniffer) {
        ...
        $httpBackend.when('GET', '/core/codename/k008trhk/123456/00').respond([record1]);
        $httpBackend.when('GET', '/core/codename/k008trhk/111111/00').respond([]);
        ...
        compile = function(label, partition, id, name, ngModel, required, readOnly, notExist, para01, id2, name2, ngModel2) {

            var htmlText = '<cc-trikmkcd-input' + ' cc-label="' + label + '" cc-partition="' + partition + '" id="'
                    + id + '" name="' + name + '" ng-model="' + ngModel
                    + ((required != null) ? ('" cc-required="' + required) : "")
                    + ((readOnly != null && readOnly == "true") ? '" cc-readonly="true' : "")
                    + ((notExist != null) ? ('" cc-notexist="' + notExist) : "")
    ...
    describe('display html correctly cases', function() {
        ...
        describe('1st div cases', function() {
            it("should render html of 1st div correctly with required set to true", function() {
                compile("取引項目", "3", "kakToriKmk", "kakToriKmk", "result.kakToriKmk", "true", null, "true", "result.kaisyaCd",
                        "toriNm", "toriNm", "result.toriNm");
                ...
                expect(element.find('div').eq(0).clone().children().remove().end().text()).toEqual("取引項目");
            ...
            it("should render html of 1st div correctly with required set to false", function() {
                compile("取引項目", "3", "kakToriKmk", "kakToriKmk", "result.kakToriKmk", 'false', null, "true", "result.kaisyaCd",
                        "toriNm", "toriNm", "result.toriNm");
                expect(element.find('div').length).toEqual(1);
            ...
            it("should render html of 1st div correctly with readonly set to true", function() {
                compile("取引項目", "3", "kakToriKmk", "kakToriKmk", "result.kakToriKmk", null, "true", "true", "result.kaisyaCd",
                        "result.jigyobuCd", "result.hakkoDay", "toriNm", "result.toriNm");
                expect(element.find('div').length).toEqual(1);
                expect(element.find('div').eq(0).hasClass("cs-label-large cs-label")).toEqual(true);
                expect(element.find('div').eq(0).clone().children().remove().end().text()).toEqual("取引項目");
                expect(element.find('div span').length).not.toEqual(1);
            ...
            it("should render html of 1st div correctly with partition set to empty", function() {
                compile("取引項目", "", "kakToriKmk", "kakToriKmk", "result.kakToriKmk", "true", null, "true", "result.kaisyaCd",
                        "toriNm", "toriNm", "result.toriNm");
                expect(element.find('div').length).toEqual(1);
        ...
        describe('1st input field', function() {
            ...
            it("should render html correctly when required", function() {
                ...
                expect(element.find('input').eq(0).attr('id')).toEqual("kakToriKmk");
                expect(element.find('input').eq(0).attr('name')).toEqual("kakToriKmk");
            ...
            it("should render html correctly when readonly", function() {
                compile("取引項目", "3", "kakToriKmk", "kakToriKmk", "result.kakToriKmk", null, "true", null, "result.kaisyaCd",
                        "toriNm", "toriNm", "result.toriNm");
        ...
        describe('2nd input field', function() {
            ...
            it("should render html correctly", function() {
                ...
                expect(element.find('input').eq(1).attr('id')).toEqual("toriNm");
                expect(element.find('input').eq(1).attr('name')).toEqual("toriNm");
     ...
     describe("logic cases", function() {
        it('it should bind value and view value correctly', function() {
            ...
            expect(inputElement.val()).toEqual('123456');
            expect(scope.result.kakToriKmk).toEqual('123456');
        ...
        it("it should be catch required validation", function() {
            ...
            inputElement.triggerHandler('blur');
            expect(scope.result.kakToriKmk).toEqual(undefined);
        ...
        it('it should be catch minlength validation ', function() {
            compile("取引項目", "3", "kakToriKmk", "kakToriKmk", "result.kakToriKmk", "true", null, "true", "result.kaisyaCd",
             ...
             inputElement.triggerHandler('focus');
             changeInputValue(inputElement, '00000');
        ...
        it('it should populate input no2 with data ', function() {
            ...
            inputElement.triggerHandler('blur');
            expect(inputElement2.val()).toEqual('買掛金）');
            expect(scope.result.toriNm).toEqual('買掛金）');
        ...
        it('it should not populate input no2 with data and display error message when there is no such data in database and notExist set to false', function() {
            compile("取引項目", "3", "kakToriKmk", "kakToriKmk", "result.kakToriKmk", "true", null, "false", "result.kaisyaCd",
                    "toriNm", "toriNm", "result.toriNm");
            inputElement = element.find('input').eq(0);
            inputElement2 = element.find('input').eq(1);
            var modelCtrl = inputElement.controller('ngModel');
            ...
            expect(scope.result.toriNm).toEqual(undefined);
            expect(modelCtrl.$invalid).toBe(true);
