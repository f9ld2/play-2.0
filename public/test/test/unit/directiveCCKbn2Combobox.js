...
//            scope.$digest();
    ...
    describe('cc-kbn2-combobox', function() {
        ...
        it('it should render right html', function() {
            ...
            expect(selectElement.attr('id')).toEqual("tanaMm");
            expect(selectElement.attr('name')).toEqual("tanaMm");
...
//            expect(selectElement.hasClass('cs-char-25')).toBe(true);
            ...
            // Test partition == ''
            compile('棚卸月','','tanaMm','tanaMm','cond.tanaMm','true',null,'2','X001');
            ...
            // Test required == 'false'
            compile('棚卸月','3','tanaMm','tanaMm','cond.tanaMm','false',null,'2','X001');
            ...
            // Test readonly = true
            compile('棚卸月','','tanaMm','tanaMm','cond.tanaMm',null,'true','2','X001');
            //-----------it should hasn't span element
            expect(element.has('div').length).toEqual(1);
            expect(element.has('span').length).toEqual(0);
            expect(element.has('select').length).toEqual(1);
        ...
        it('it should be right binding value and view value', function() {
            // shortName = false
            compile('棚卸月','3','tanaMm','tanaMm','cond.tanaMm','false',null,'3','X002');
            $httpBackend.flush();
            selectElement = angular.element(element.children()[1]);
            ...
            expect(angular.element(children[1]).text()).toEqual('0000001');
            expect(angular.element(children[2]).attr('value')).toEqual('0000002');
            expect(angular.element(children[2]).text()).toEqual('0000002');
            expect(angular.element(children[3]).attr('value')).toEqual('0000003');
            expect(angular.element(children[3]).text()).toEqual('0000003');
            ...
            expect(scope.cond.tanaMm).toEqual('');
...
//            expect(selectElement.text()).toEqual('');
            ...
            expect(scope.cond.tanaMm).toEqual('0000002');
            ...
            // shortName = true
            compile('棚卸月','3','tanaMm','tanaMm','cond.tanaMm','true',null,'3','X001');
            $httpBackend.flush();
            selectElement = angular.element(element.children()[1]);
            //-----------select second option
            changeSelectValue(selectElement, 1);
            //-----------binding value should is 0000001 after selecting
            expect(scope.cond.tanaMm).toEqual('0000001');
            ...
            expect(angular.element(children[2]).text()).toEqual('0000009');
            
            // hakkoDay = 20140101
            scope.cond.hakkoDay = '20140101'
            compile('棚卸月','3','tanaMm','tanaMm','cond.tanaMm','true',null,'3','X002');
            $httpBackend.flush();
            selectElement = angular.element(element.children()[1]);
            var children = selectElement.children();
        ...
        it('it should be catch ccSelect and required validation ', function() {
            ...
            // value of scope.cond.tanaMm out of listRecord
            scope.cond.tanaMm = '0000000';
//            scope.$digest();
