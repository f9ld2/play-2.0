describe('Test cc-mtori-input', function() {
    ...
    beforeEach(inject(function($injector, $sniffer) {
        ...
        $httpBackend.when('GET', '/core/codename/m011trim2/567890?hakkoDay=19010101').respond([record1]);
        $httpBackend.when('GET', '/core/codename/m011trim2/567890?hakkoDay=20140101').respond([record3]);
        $httpBackend.when('GET', '/core/codename/m011trim2/123456?hakkoDay=19010101').respond([record2]);
        $httpBackend.when('GET', '/core/codename/m011trim2/111111?hakkoDay=19010101').respond([]);
        ...
        compile = function(label, partition, id, name, ngModel, required, readOnly, notExist, delexist, shortName,
            ...
            var htmlText = '<cc-mtoricd-input' + ' cc-label="' + label + '" cc-partition="' + partition + '" id="'
                    ...
                    + '" cc-shortname="' + shortName + '" cc-parameter01="' + para01 
    ...
    describe('display html correctly cases', function() {
        ...
        describe('1st div cases', function() {
            it("should render html of 1st div correctly with required set to true", function() {
                compile("取引先", "3", "triCd", "triCd", "result.triCd", "true", null, "true", "false", "false", "result.hakkoDay",
                        "triNm", "triNm", "result.triNm");
            ...
            it("should render html of 1st div correctly with required set to false", function() {
                compile("取引先", "3", "triCd", "triCd", "result.triCd", 'false', null, "true", "false", "false", "result.hakkoDay",
                        "triNm", "triNm", "result.triNm");
                expect(element.find('div').length).toEqual(1);
            ...
            it("should render html of 1st div correctly with readonly set to true", function() {
                compile("取引先", "3", "triCd", "triCd", "result.triCd", null, "true", "true", "false", "false", "result.hakkoDay",
                        "triNm", "triNm", "result.triNm");
                expect(element.find('div').length).toEqual(1);
                expect(element.find('div').eq(0).hasClass("cs-label-large cs-label")).toEqual(true);
                expect(element.find('div').eq(0).clone().children().remove().end().text()).toEqual("取引先");
            ...
            it("should render html of 1st div correctly with partition set to empty", function() {
                compile("取引先", "", "triCd", "triCd", "result.triCd", "true", null, "true", "false", "false", "result.hakkoDay",
                        "triNm", "triNm", "result.triNm");
                expect(element.find('div').length).toEqual(1);
        ...
        describe('1st input field', function() {
            ...
            it("should render html correctly when required", function() {
                ...
                expect(element.find('input').eq(0).attr('id')).toEqual("triCd");
                expect(element.find('input').eq(0).attr('name')).toEqual("triCd");
                ...
                expect(element.find('input').eq(0).attr('ng-minlength')).toEqual("6");
                ...
                expect(element.find('input').eq(0).attr('maxlength')).toEqual("6");
            });
            
            it("should render html correctly when readonly", function() {
                compile("取引先", "3", "triCd", "triCd", "result.triCd", null, "true", null, "false", "false", "result.hakkoDay",
                        "triNm", "triNm", "result.triNm");
        ...
        describe('2nd input field', function() {
            ...
            it("should render html correctly", function() {
                ...
                expect(element.find('input').eq(1).attr('id')).toEqual("triNm");
                expect(element.find('input').eq(1).attr('name')).toEqual("triNm");
                ...
                compile("取引先", "3", "triCd", "triCd", "result.triCd", "true", null, "true", "false", "true", "result.hakkoDay",
                        "triNm", "triNm", "result.triNm");
     ...
     describe("logic cases", function() {
        it('it should bind value and view value correctly', function() {
            compile("取引先", "3", "triCd", "triCd", "result.triCd", "true", null, "true", "true", "true", "result.hakkoDay",
                    "triNm", "triNm", "result.triNm");
            inputElement = element.find('input').eq(0);
            var modelCtrl = inputElement.controller('ngModel');
            
            inputElement.triggerHandler('focus');
            changeInputValue(inputElement, '567890');
            $httpBackend.flush();
            
            inputElement.triggerHandler('blur');
            expect(inputElement.val()).toEqual('567890');
            expect(scope.result.triCd).toEqual('567890');
        ...
        it('it should populate input no2 with data, hakkoday exists ', function() {
            ...
            inputElement.triggerHandler('blur');
            expect(inputElement2.val()).toEqual('（株）アールエス飯田1');
            expect(scope.result.triNm).toEqual('（株）アールエス飯田1');
        ...
        it('it should populate input no2 with data, hakkoday doesnot exist ', function() {
            ...
            inputElement.triggerHandler('blur');
            expect(inputElement2.val()).toEqual('（株）アールエス飯田');
            expect(scope.result.triNm).toEqual('（株）アールエス飯田');
        ...
        it('it should not populate input no2 with data and display error message when there is no such data in database and notExist set to false', function() {
            compile("取引先", "3", "triCd", "triCd", "result.triCd", "true", null, "false", "false", "false", "result.hakkoDay",
                    "triNm", "triNm", "result.triNm");
            inputElement = element.find('input').eq(0);
            inputElement2 = element.find('input').eq(1);
            var modelCtrl = inputElement.controller('ngModel');
            ...
            inputElement.triggerHandler('focus');
            changeInputValue(inputElement, '111111');
            $httpBackend.flush()
            
            inputElement.triggerHandler('blur');
            expect(inputElement2.val()).toEqual('');
            expect(scope.result.triNm).toEqual(undefined);
            expect(modelCtrl.$invalid).toBe(true);
        ...
        it('it should not populate input no2 with data and display message when there is such data with kubun 9 and notDelexist set to false', function() {
            ...
            inputElement.triggerHandler('focus');
            changeInputValue(inputElement, '123456');
            $httpBackend.flush()
