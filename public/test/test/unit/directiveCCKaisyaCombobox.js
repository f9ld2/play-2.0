...
describe('Test directive', function() {
    ...
    beforeEach(inject(function($injector, $sniffer) {
        ...
        list1 = [{code:'0000001',name:'メーカー名称（漢字）1',shortName:'メーカー名略称1',kubun:'1'},
                 {code:'0000009',name:'メーカー名称（漢字）9',shortName:'メーカー名略称9',kubun:'9'}];
        list2 = [{code:'1000001',name:'メーカー名称（漢字）1',shortName:'メーカー名略称1',kubun:'1'},
                 {code:'1000002',name:'メーカー名称（漢字）2',shortName:'メーカー名略称2',kubun:'2'},
                 {code:'1000003',name:'メーカー名称（漢字）3',shortName:'メーカー名略称3',kubun:'3'}];
        $httpBackend.when('GET', '/core/codemaster/m000kaim?hakkoDay=19010101').respond(list1);
        $httpBackend.when('GET', '/core/codemaster/m000kaim?hakkoDay=20140101').respond(list2);
    ...
    describe('cc-kaisya-combobox', function() {
        ...
        // test template
        xit('it should render right html', function() {

            // Test partition != '', required = true, shortName = false, delexist = false
            compile('会社','3','kaisyaCd','kaisyaCd','cond.kaisyaCd','true',null,'false','false', 'cond.hakkoDay');
            ...
            expect(selectElement.attr('id')).toEqual("kaisyaCd");
            expect(selectElement.attr('name')).toEqual("kaisyaCd");
            ...
            // Test partition == ''
            compile('会社','','kaisyaCd','kaisyaCd','cond.kaisyaCd','true',null,'false','false', 'cond.hakkoDay');
            ...
            // Test required == 'false'
            compile('会社','3','kaisyaCd','kaisyaCd','cond.kaisyaCd','false',null,'false','false', 'cond.hakkoDay');
            ...
            // Test readonly = true
            compile('会社','','kaisyaCd','kaisyaCd','cond.kaisyaCd',null,'true','false','false', 'cond.hakkoDay');
            //-----------it should hasn't span element
            expect(element.has('div').length).toEqual(1);
            expect(element.has('span').length).toEqual(0);
            expect(element.has('select').length).toEqual(1);
            ...
            // Test shortName = true
            compile('会社','3','kaisyaCd','kaisyaCd','cond.kaisyaCd','true',null,'false','true', 'cond.hakkoDay');
        ...
        it('it should be right binding value and view value', function() {
            ...
            expect(scope.cond.kaisyaCd).toEqual('');
...
//            expect(selectElement.text()).toEqual('');
            ...
            expect(scope.cond.kaisyaCd).toEqual(list1[0].code);
            ...
            // delexist = false
            compile('会社','3','kaisyaCd','kaisyaCd','cond.kaisyaCd','true',null,'true','false', 'cond.hakkoDay');
            $httpBackend.flush();
            selectElement = angular.element(element.children()[1]);
            var children = selectElement.children();
        ...
        it('it should be catch ccSelect and required validation ', function() {
            ...
            // value of scope.cond.kaisyaCd out of listRecord
            scope.cond.kaisyaCd = '0000000';
            //scope.$digest();
