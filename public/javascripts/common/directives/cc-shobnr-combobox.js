/**
 * @ngdoc directive
 * @name chasecore.cc-shobnr-combobox
 * @restrict E
 * @function
 *
 * @description
 * (部品生成時)分類マスタから『クラス＋(クラス)分類名称(or略称)』を取得・設定する
 *
 * @example
    <doc:example>
      <doc:source>
         <cc-shobnr-combobox 
            cc-label="クラス" cc-partition="3" id="shobnrCd" name="shobnrCd"
            ng-model="result.shobnrCd" cc-required=false cc-delexist=false
            cc-shortname=true cc-parameter01="result.jigyobuCd" 
            cc-parameter02="result.bmnCd" cc-parameter03="result.chubnrCd"
...
diretiveApp.directive('ccShobnrCombobox', function(CSS_CLASS, $http, $compile, $timeout, UserInfo, $rootScope, ClearComboboxOptions, AddComboboxOptions) {
    var unyoDate = UserInfo.unyoDate;

    function getShobnrDataList(jigyobuCd, bmnCd, chubnrCd, hakkoDay, selectElement, attrs, element, scope) {
        //未設定の場合は運用日付を取得して代用する。
        if (isEmpty(hakkoDay)) {
            getShobnrList(jigyobuCd, bmnCd, chubnrCd, unyoDate, selectElement, attrs, element, scope);
        } else {
            getShobnrList(jigyobuCd, bmnCd, chubnrCd, hakkoDay, selectElement, attrs, element, scope);
        }
    }

    function getShobnrList(jigyobuCd, bmnCd, chubnrCd, hakkoDay, selectElement, attrs, element, scope) {
        var url = '/core/codename/m005bnrm5/' + jigyobuCd + '/' + bmnCd + '/' + chubnrCd + '?hakkoDay=' + hakkoDay;
    ...
    return {
        ...
        link: function(scope, element, attrs, ctrl) {
            ...
            var validate = function(jigyobuCd, bmnCd, chubnrCd, hakkoDay, modelValue) {
                if (isEmpty(jigyobuCd) || isEmpty(bmnCd) || isEmpty(chubnrCd)) {
            ...
            var loadData = function() {
                ...
                var param3 = scope.ccParameter03; // chubnrCd
                // hakkoDay
                var param4 = scope.ccParameter04;
                if (isEmpty(param1) || isEmpty(param2) || isEmpty(param3)) {
                    ClearComboboxOptions(scope, attrs);
                } else {
                    getShobnrDataList(param1, param2, param3, param4, selectElement, attrs, element, scope);
                }
            }
