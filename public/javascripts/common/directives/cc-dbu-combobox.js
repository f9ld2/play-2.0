/**
 * @ngdoc directive
 * @name chasecore.cc-dbu-combobox
 * @restrict E
 * @function
 *
 * @description
 * (部品生成時)大部門マスタから『大部門コード＋大部門名称(or略称)』を取得・設定する
 *
 * @example
    <doc:example>
      <doc:source>
         <cc-dbu-combobox 
            cc-label="大部門" cc-partition="3" id="dbuCd" name="dbuCd"
            ng-model="result.dbuCd" cc-required=false cc-delexist=false
...
diretiveApp.directive('ccDbuCombobox', function(CSS_CLASS, $http, $compile, UserInfo, ClearComboboxOptions, AddComboboxOptions) {
    var unyoDate = UserInfo.unyoDate;

    function getDBUDataList(hakkoDay, selectElement, attrs, element, scope) {
    ...
    function getDataList(hakkoDay, selectElement, attrs, element, scope) {
        var url = '/core/codename/m002dbum2' + '?hakkoDay=' + hakkoDay;
    ...
    return {
        ...
        link: function(scope, element, attrs, ctrl) {
            ...
            var loadData = function() {
                getDBUDataList(scope.ccParameter01, selectElement, attrs, element, scope);
            }
